...
| Playwright compatibility starter | `scripts/start-workbench-playwright-backend.sh` is the active backend start script referenced by `frontend/playwright.config.ts`; `scripts/start-template-playwright-backend.sh` is a deprecation shim. | Retain the shim temporarily for older local commands; new browser tooling must use the Workbench-named script. |
