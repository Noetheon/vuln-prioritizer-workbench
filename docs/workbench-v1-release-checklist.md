...
- Duplicate VPW-076 release-story closeout evidence is archived in `archive/vpw-evidence/vpw-076-release-story.md`.
...
- [x] Archive the machine-readable result under `archive/vpw-evidence/vpw-072-performance-smoke.json`.
...
- [x] If `pip-audit`, npm, or advisory data is unavailable, record the failure as a release exception with date, environment, and retry decision.
...
| Dependency audit | `make dependency-audit`; current gate covers Python lock hygiene, `backend/requirements.lock.txt`, `backend/requirements.runtime.lock.txt`, and `frontend/package-lock.json` production dependencies | Codex technical validation | 2026-04-25 |
