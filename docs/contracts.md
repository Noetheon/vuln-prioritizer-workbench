...
- Workbench service-token scopes are enforced only when an explicit `vpr_` bearer token is supplied: `read` covers project/run/finding/provider/waiver reads, `write` covers project-scoped project/asset/waiver mutations, `import` covers `/api/v1/projects/{project_id}/imports`, `report` covers report creation/download/verification, and `admin` covers token lifecycle, project creation, and satisfies all scoped dependencies; revoked, expired, or inactive-user tokens are rejected
- the current Workbench access model is local single-user browser access plus optional project-scoped non-admin service tokens; project membership tables and project-admin roles are not implemented in this local-first release
- legacy browser JWT endpoints remain available for backend compatibility, but the active browser UI does not require login; `/api/v1/login/logout` clears/revokes a browser session token only when such a token is used
- `/api/v1/audit/events` lists redacted audit events for administrators, and `/api/v1/audit/sessions` lists legacy browser session metadata without JWT IDs, token hashes, or cleartext secrets
...
- `POST /api/v1/projects/{project_id}/imports` accepts a local Workbench upload, validates input type, extension, MIME hint, filename, and upload size, persists the uploaded file under the configured import upload root, and records upload SHA-256 plus structured `parse_errors` in the returned `AnalysisRun`
...
- `GET /api/v1/providers/status` returns a local provider-status envelope for the React status card, including `status`, `snapshot`, `sources`, `latest_update_job`, `cache_dir`, `snapshot_dir`, `warnings`, `last_sync`, `last_error`, `cache_age_seconds`, and `snapshot_mode`
