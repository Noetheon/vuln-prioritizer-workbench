...
- `POST /api/v1/api-tokens/`, `GET /api/v1/api-tokens/`, and `DELETE /api/v1/api-tokens/{token_id}` manage Workbench service tokens; the create response is the only response that includes the cleartext token
...
- Workbench imports run the shared parse/enrich/score/explain engine before findings are persisted; stored findings include the effective priority, priority rank, risk score, operational rank, KEV/EPSS/CVSS signals, lifecycle flags, rationale, recommended action, and structured `explanation_json`
...
- `GET /api/v1/runs/{run_id}/summary` returns a UI-oriented summary with stable `created_findings`, `updated_findings`, `ignored_lines`, `occurrence_count`, `finding_count`, `parse_errors`, `import_job`, `input_upload`, `dedup_summary`, `counts_by_priority`, `kev_hits`, `provider_snapshot_id`, and `provider_degraded` fields
...
- `GET /api/v1/projects/{project_id}/compare/cvss-only` returns the CVSS-only baseline comparison for stored Workbench findings, using the same methodology payload as the core decision engine
