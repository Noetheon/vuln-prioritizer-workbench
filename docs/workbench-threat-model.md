...
| Docker demo smoke | `docker compose -f compose.yml -f compose.override.yml up --build backend frontend` is the supported active Workbench readiness path. | `make docker-demo-smoke` starts Compose, polls `http://127.0.0.1:8000/api/v1/utils/health-check/`, runs an authenticated import/provider smoke, and tears the stack down with `docker compose down -v --remove-orphans`. |
| Production-like Docker smoke | `compose.production-smoke.yml` overlays production environment flags, non-default secrets, same-origin frontend API routing, docs-disabled backend behavior, exact hosts, and secure cookies. | `make docker-production-smoke` validates CSP, minimal public health, auth-gated diagnostics, HttpOnly/SameSite/Secure session cookies, CSRF mutation enforcement, import, report download, logout revocation, and path-redacted JSON responses. |
...
- `make docker-demo-smoke` for the Compose quickstart path; it starts the Workbench stack, polls `/api/v1/utils/health-check/`, verifies a locked provider-data import, triggers `/api/v1/providers/update-jobs`, and removes the demo stack.
- `make docker-production-smoke` for the public-production path; it starts the production overlay, validates same-origin CSP/API routing, auth-gated diagnostics, secure cookies, CSRF, import, report download, logout, and path redaction.
