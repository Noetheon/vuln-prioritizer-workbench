...
| Traefik dashboard exposed by default | Administrative proxy metadata or controls reachable from an unintended network | The Compose Traefik dashboard router is disabled unless `TRAEFIK_DASHBOARD_ENABLED=true`. If enabled, keep `TRAEFIK_DASHBOARD_IP_ALLOWLIST` narrowed to the operator network and set operator-owned `TRAEFIK_DASHBOARD_AUTH_USERS` credentials. The Compose default uses a non-operational placeholder Basic Auth user instead of opening the dashboard without credentials. The Traefik container also runs with a read-only root filesystem, `no-new-privileges`, dropped capabilities, and only `NET_BIND_SERVICE` restored for public low-port binding. |
...
| Duplicate or repeated GitHub issue creation | Alert fatigue, duplicate remediation work, or noisy external audit trails | Generate deterministic duplicate keys and enforce one reservation per project/repository/finding in the database. Commit the reservation before the external POST. Definitive client rejections release it; invalid success bodies, network failures, rate-limit/time-out responses, server-side 5xx responses, and process crashes retain it and fail closed with HTTP 409 on retry, because none proves that the remote side effect did not occur. Remote reconciliation is not automatic: after verifying GitHub by the embedded duplicate-key marker, an operator must deliberately repair or clear an unresolved local reservation. |
...
- The active API-token, login, user-management, and session-list surfaces are
  disabled. The v2 initial Workbench schema does not create inactive user,
  API-token, or auth-session tables, so they are not an active automation,
  persistence, or browser-login model.
...
- GitHub issue preview/export is an optional local automation bridge. Preview
  ...
  environment variable name. An incomplete export reservation blocks project
  deletion with HTTP 409, so an in-flight or ambiguous remote create cannot be
  erased together with its local idempotency record.
...
- disabled Traefik dashboard routing, or dashboard routing protected by a narrow IP allowlist and authentication middleware, plus reverse-proxy container hardening
...
- operator docs state that internet exposure, multi-tenancy, SSO, multi-node
  worker fleets, and organization-wide ticket sync policy are outside the
...
| 10k findings API smoke | The active API exposes paginated findings with limit/offset and sort controls; global reranking uses bounded keyset batches. This is a smoke check, not a concurrent-user or PostgreSQL benchmark. | `make performance-smoke` imports 10,000 findings, checks tail pagination, then adds one scope and proves 10,001 contiguous unique operational ranks within separate 60-second phase limits and a 512 MiB RSS-delta limit. |
