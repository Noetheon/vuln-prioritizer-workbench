...
| API token misuse | Unauthorized state changes through local API routes | Active `/api/v1` browser and service-token routes run through `backend/app`, require JWT login before service-token creation, and enforce `read`, `write`, `import`, `report`, or `admin` scopes. Store only token hashes, update last-used metadata, and do not render token values again after creation. |
