...
- bootstrap default secrets are accepted only for local/dev bootstrap and are rejected for staging and production
...
| API token scopes | Workbench token routes in `backend/app/api/routes/api_tokens.py` create one-time token values after JWT login, store PBKDF2-SHA256 hashes, and expose only metadata after creation. Workbench auth dependencies in `backend/app/api/deps.py` enforce `read`, `write`, `import`, `report`, and `admin` scopes. | `backend/tests/api/test_template_api_tokens.py` verifies hash storage, one-time display, scoped read/write/import/report/admin behavior, `last_used_at`, and revoked-token rejection. `backend/tests/test_backend_runtime_boundary.py` verifies the active import graph does not reach removed runtime modules. |
