...
| Optional Postgres profile | `compose.yml` starts the template backend and frontend by default and keeps `workbench-postgres` under the `postgres` profile with a private `127.0.0.1:8001` bind for legacy Alembic migration smoke coverage. | `tests/test_workbench_integration_contracts.py` checks the profile contract; `make docker-postgres-migration-smoke` exercises the profile and Alembic path when Docker is available. |
