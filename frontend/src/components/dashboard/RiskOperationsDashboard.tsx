...
function latestRunLabel(run: AnalysisRunPublic | null) {
  ...
  return run.started_at ? `${statusLabel} · ${formatDateTime(run.started_at)}` : statusLabel
}
...
export function RiskOperationsDashboard({
  ...
  const effectiveProviderStatus = isDemoMode ? DEMO_PROVIDER_STATUS : providerStatus
  ...
  const queueFindings = useMemo(() => {
    const ranked = [...effectiveFindings].sort((a, b) => (b.risk_score ?? 0) - (a.risk_score ?? 0))
  ...
  const freshnessCard = useMemo(
    () => [
      ...
      },
      {
        ...
        tone: freshness.tone,
        value: (
          <span className="text-sm font-semibold">{freshness.value}</span>
        ),
      },
      {
  ...
  return (
    <TooltipProvider>
    <section aria-label="Risk Operations dashboard" className="space-y-4 pb-4">
      {/* ── HERO ── */}
      <div className="relative overflow-hidden rounded-xl border border-slate-700/50 bg-linear-to-br from-slate-900 via-slate-800 to-slate-950 px-5 py-4 shadow-lg">
        <div
          aria-hidden="true"
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="truncate text-base font-semibold text-white">
              {effectiveSelectedProject ? effectiveSelectedProject.name : "No project selected"}
            </p>
        ...
        <div className="relative mt-3 flex flex-wrap items-center gap-3 border-t border-white/10 pt-3">
          <ProviderStatusBadge
            status={effectiveProviderStatus?.status ?? (providerStatusLoading ? "loading" : "unknown")}
          />
      ...
      {isDemoMode ? (
        <div className="flex items-center gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-800">
          <ShieldCheck aria-hidden="true" className="size-4 shrink-0 text-amber-600" />
          <p>
            <strong className="font-semibold">Demo preview</strong> — sample data from a fictional
            payments service. Connect a real project to see live metrics.
          </p>
      ...
      {staleProvider ? (
        <Card className="border-amber-300 bg-amber-50/70 dark:bg-amber-950/35">
          <CardHeader className="py-3">
            ...
            <CardDescription className="text-xs text-amber-700/80 dark:text-amber-400/80">
              Freshness is stale or partially degraded. Remediation priority remains functional,
              but evidence may not be fully current.
            </CardDescription>
          </CardHeader>
        </Card>
      ) : null}
      ...
      {showEmptyState ? (
        <div
          ...
          className="rounded-xl border border-slate-200 bg-linear-to-br from-white via-slate-50/80 to-slate-100/60 px-8 py-10"
        >
          <div className="mx-auto max-w-lg">
            <div className="flex flex-col items-center text-center">
              <div className="mb-4 flex size-14 items-center justify-center rounded-2xl bg-slate-900 shadow-md">
                <ShieldCheck className="size-7 text-amber-400" />
              </div>
              <h3 className="text-lg font-semibold text-slate-900">Set up Risk Operations</h3>
              <p className="mt-2 max-w-sm text-sm text-muted-foreground">
                Select a project, import findings, and run analysis to activate the full Risk Operations cockpit.
              </p>
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <div className="mx-auto mb-2 flex size-7 items-center justify-center rounded-full bg-slate-100 text-xs font-bold text-slate-600">1</div>
                <p className="text-xs font-medium text-slate-700">Select or create a project</p>
              </div>
              <div>
                <div className="mx-auto mb-2 flex size-7 items-center justify-center rounded-full bg-slate-100 text-xs font-bold text-slate-600">2</div>
                <p className="text-xs font-medium text-slate-700">Import scanner findings</p>
              </div>
              <div>
                <div className="mx-auto mb-2 flex size-7 items-center justify-center rounded-full bg-slate-100 text-xs font-bold text-slate-600">3</div>
                <p className="text-xs font-medium text-slate-700">Run analysis to prioritize</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="min-w-0 space-y-4">
          ...
          <Card className="gap-2 py-4">
            <CardHeader>
              ...
              <CardDescription>
                Signal concentration, service risk, and trend direction for executive review.
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="gap-2 py-4">
            <CardHeader className="px-4">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <CardTitle>Top Remediation Queue</CardTitle>
                  <CardDescription>
            ...
            <CardContent className="px-0 pb-2">
              {findingsLoading ? (
                <div className="px-6"><Skeleton className="h-64" /></div>
              ) : findingsError ? (
                <div className="px-6"><ErrorState message={findingsError} /></div>
              ) : queueFindings.length === 0 ? (
                <div className="px-6"><EmptyState
                  ...
                  title={filters.queueSearch ? "No matching findings" : "No findings"}
                /></div>
              ) : (
                        ...
                        <TableHead className="w-20">
                          <Tooltip>
                            ...
                            <TooltipContent side="top" className="max-w-56 text-xs">
                              Composite score (0–10) combining CVSS severity, EPSS exploitation probability, KEV status, and asset exposure.
                            </TooltipContent>
                          </Tooltip>
                        </TableHead>
                            ...
                            <Dialog>
                              ...
                              <DialogContent>
                                <DialogHeader>
                                  ...
                                  <DialogDescription>
                                    Current priority rationale from scoring and operational
                                    context.
                                  </DialogDescription>
                                </DialogHeader>
                          ...
                          <TableCell>
                            <Sheet>
                              ...
                              <SheetContent className="overflow-y-auto w-[380px] sm:w-[460px]">
                                <SheetHeader>
                                  <SheetTitle className="font-mono">{finding.cve_id ?? "Finding"}</SheetTitle>
                                  <SheetDescription>Quick view — open full detail for complete context.</SheetDescription>
                                </SheetHeader>
                                <div className="mt-6 space-y-5">
                                  ...
                                  <dl className="space-y-3 text-sm">
                                    <div>
                                      <dt className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Component</dt>
                                    ...
                                    <div>
                                      <dt className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Why now</dt>
                                      <dd className="mt-0.5 text-muted-foreground">{findingWhyNow(finding)}</dd>
                                    </div>
                                  </dl>
        ...
        <div className="space-y-3">
          ...
          <Card className="gap-3 py-4">
            <CardHeader className="pb-0 px-4">
              <CardTitle className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Provider Freshness</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2 px-4">
              <div className="flex items-center gap-2">
                <ProviderStatusBadge
                  status={
                    effectiveProviderStatus?.status ?? (providerStatusLoading ? "loading" : "unknown")
                  }
                />
                <span className="text-sm font-bold">{freshness.value}</span>
              </div>
              <p className="text-xs leading-relaxed text-muted-foreground">{freshness.detail}</p>
          ...
          <Card className="gap-3 py-4">
            <CardHeader className="pb-0 px-4">
              <CardTitle className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Evidence Readiness</CardTitle>
            </CardHeader>
            <CardContent className="px-4">
              <dl className="space-y-2 text-sm">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <dd className="font-semibold">{effectiveSummary?.finding_count ?? 0}</dd>
                </div>
                <div className="flex items-center justify-between">
          ...
          <Card className="gap-3 py-4">
            <CardHeader className="pb-0 px-4">
              <CardTitle className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Data Quality</CardTitle>
            </CardHeader>
            <CardContent className="px-4">
                ...
                <ul className="space-y-2">
                  {dataQualityWarnings.map((warning) => (
                    <li className="flex gap-2 text-xs" key={warning}>
                      ...
                      />
                      <span className="leading-relaxed text-muted-foreground">{warning}</span>
                    </li>
                  ))}
                  {dataQualityError && (
          ...
          <Card className="gap-3 py-4">
            <CardHeader className="pb-0 px-4">
              <CardTitle className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Recent Runs</CardTitle>
            </CardHeader>
            <CardContent className="px-4">
                ...
                <div className="space-y-2">
                  {latestRunFactsRows.map((run) => (
                    ...
                    >
                      <Badge
                        className="shrink-0"
                        variant={run.tone === "succeeded" ? "secondary" : "destructive"}
                      >
                        {run.status}
                      </Badge>
                      <span className="truncate text-muted-foreground">{run.startedAt}</span>
                    </div>
                  ))}
                </div>
              )}
