...
PYTHON_LOCK = Path(__file__).resolve().parents[2] / "uv.lock"
PYTHON_AUDIT_LOCK = Path(__file__).resolve().parents[2] / "backend" / "requirements.lock.txt"
PYTHON_RUNTIME_LOCK = (
    Path(__file__).resolve().parents[2] / "backend" / "requirements.runtime.lock.txt"
)
DEPENDENCY_POLICY_DOC = (
    Path(__file__).resolve().parents[2] / "docs" / "dependency-and-package-policy.md"
)
...
def test_frontend_dependency_audit_uses_reproducible_npm_lockfile() -> None:
    ...
    policy_doc = DEPENDENCY_POLICY_DOC.read_text(encoding="utf-8")
    python_lock = PYTHON_LOCK.read_text(encoding="utf-8")
    python_audit_lock = PYTHON_AUDIT_LOCK.read_text(encoding="utf-8")
    python_runtime_lock = PYTHON_RUNTIME_LOCK.read_text(encoding="utf-8")
    ...
    assert "[tool.uv.workspace]" not in python_lock
    assert 'name = "vuln-prioritizer"' in python_lock
    assert "autogenerated by uv" in python_audit_lock
    assert "autogenerated by uv" in python_runtime_lock
    assert "--python 3.12" in python_runtime_lock
    assert "--no-dev" in python_runtime_lock
    assert "PYTHON_AUDIT_LOCK := $(BACKEND_DIR)/requirements.lock.txt" in makefile
    assert "PYTHON_RUNTIME_LOCK := $(BACKEND_DIR)/requirements.runtime.lock.txt" in makefile
    assert "--requirement $(PYTHON_AUDIT_LOCK)" in dependency_audit_block
    assert "python-lock-check" in dependency_audit_block
    assert "backend/requirements.txt" in policy_doc
    assert "bounded Python audit policy input" in policy_doc.replace("\n", " ")
    assert "uv.lock" in policy_doc
    assert "backend/requirements.lock.txt" in policy_doc
    assert "backend/requirements.runtime.lock.txt" in policy_doc
    assert not re.search(
        r"^(mkdocs|mypy|pip-audit|pytest|pytest-cov|ruff|twine)==",
        python_runtime_lock,
        re.MULTILINE,
    )
