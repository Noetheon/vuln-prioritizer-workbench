...
def test_db_bootstrap_helpers_create_and_update_configured_superuser(tmp_path: Path) -> None:
    ...
    assert db_module.configured_superuser_id("ADMIN@example.test") == (
    ...
    with Session(engine) as session:
        db_module.init_db(session, active_settings=active_settings)
        user = session.exec(select(User).where(User.email == active_settings.FIRST_SUPERUSER)).one()
        assert user.id == db_module.configured_superuser_id(active_settings.FIRST_SUPERUSER)
        assert security_module.verify_password(
            active_settings.FIRST_SUPERUSER_PASSWORD,
            user.hashed_password,
        )

        user.hashed_password = "configured-superuser-password-placeholder"
        session.add(user)
        session.commit()

        updated_user = db_module.ensure_configured_superuser(
            session,
...
def test_db_bootstrap_integrity_race_returns_existing_or_reraises() -> None:
    active_settings = replace(
        settings,
        FIRST_SUPERUSER="race-admin@example.test",
        FIRST_SUPERUSER_PASSWORD="bootstrap-password-123",
    )
    existing_user = User(
        id=db_module.configured_superuser_id(active_settings.FIRST_SUPERUSER),
        email=active_settings.FIRST_SUPERUSER,
        is_active=True,
        is_superuser=True,
        hashed_password="already-created",
    )
    ...
    class RacingSession:
        def __init__(self, *, existing_after_rollback: User | None) -> None:
            self.existing_after_rollback = existing_after_rollback
            self.exec_calls = 0
            self.rollback_calls = 0

        def exec(self, statement: object) -> QueryResult:
            self.exec_calls += 1
            if self.exec_calls == 1:
                return QueryResult(None)
            return QueryResult(self.existing_after_rollback)
    ...
    recovered_session = RacingSession(existing_after_rollback=existing_user)
    assert (
        db_module.ensure_configured_superuser(
            recovered_session,  # type: ignore[arg-type]
            active_settings=active_settings,
        )
        is existing_user
    )
    assert recovered_session.rollback_calls == 1

    failing_session = RacingSession(existing_after_rollback=None)
...
def test_security_helpers_reject_malformed_hashes_csrf_and_jwts() -> None:
    assert not security_module.verify_password(
        "secret",
        "argon2$600000$c2FsdA$ZGlnZXN0",
    )
    assert not security_module.verify_password(
        "secret",
        "pbkdf2_sha256$0$c2FsdA$ZGlnZXN0",
    )
    assert not security_module.verify_password("secret", "not-a-complete-hash")

    assert not security_module.verify_csrf_token("jti", "missing-separator")
    assert not security_module.verify_csrf_token("jti", "nonce.")
    assert not security_module.verify_csrf_token("jti", "nonce.%")

    secret_key = "unit-test-secret"
    unsupported_alg = _signed_token(
        {"alg": "HS512", "typ": "JWT"},
        {
    ...
    missing_exp = _signed_token(
        {"alg": security_module.ALGORITHM, "typ": "JWT"},
        {"sub": "admin@example.test"},
        secret_key=secret_key,
    )
    expired = security_module.create_access_token(
        "admin@example.test",
    ...
    for token, expected_message in [
        (unsupported_alg, "Could not decode token"),
        (missing_exp, "Could not decode token"),
        (expired, "Could not decode token"),
    ]:
        with pytest.raises(security_module.TokenDecodeError, match=expected_message):
            security_module.decode_access_token(token, secret_key=secret_key)


def _signed_token(header: dict[str, object], claims: dict[str, object], *, secret_key: str) -> str:
    signing_input = ".".join(
        [
            security_module._base64url_encode(
                security_module.json.dumps(header, separators=(",", ":")).encode("utf-8")
            ),
            security_module._base64url_encode(
                security_module.json.dumps(claims, separators=(",", ":")).encode("utf-8")
            ),
        ]
    )
    signature = security_module.hmac.new(
        secret_key.encode("utf-8"),
        signing_input.encode("ascii"),
        security_module.hashlib.sha256,
    ).digest()
    return f"{signing_input}.{security_module._base64url_encode(signature)}"
...
def test_rate_limit_edges_cover_disabled_limits_and_trusted_forwarding(
    ...
    blocked = limiter.check("login:alice", limit=1)
    ...
    assert rate_limit_module.rate_limit_key(forwarded_login, proxy_settings) == (
        "login:2001:db8::8:submitted",
        3,
    )
...
def test_user_password_failure_activate_and_missing_user_paths(
    ...
    assert mismatch.json()["detail"] == "Current password is incorrect."
    ...
    activated = client.post(f"/api/v1/users/{target_id}/activate", headers=headers)
    missing = client.post(f"/api/v1/users/{uuid.uuid4()}/deactivate", headers=headers)

    assert activated.status_code == 200, activated.text
    assert activated.json()["is_active"] is True
    assert missing.status_code == 404
    with Session(template_api_env.engine) as session:
        audit_actions = [event.action for event in session.exec(select(AuditEvent)).all()]
    assert "user.password.rotate" in audit_actions
    assert "user.activate" in audit_actions


def test_audit_sessions_route_lists_sessions_without_token_material(
    template_api_env: TemplateApiEnv,
) -> None:
    client = template_api_env.client
    headers = auth_headers(client)

    response = client.get("/api/v1/audit/sessions", headers=headers)

    assert response.status_code == 200, response.text
    payload = response.json()
    assert payload["count"] >= 1
    assert payload["data"][0]["user_id"]
    assert payload["data"][0]["active"] is True
...
def test_retention_cli_commits_or_rolls_back_by_mode(
    template_api_env: TemplateApiEnv,
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(retention_module, "engine", template_api_env.engine)
