...
def test_vpw077_imports_attack_stix_snapshot_catalog_and_provider_status(
    ...
    with Session(workbench_api_env.engine) as session:
        tactics = session.exec(select(workbench_api_env.app_models.AttackStixTactic)).all()
        techniques = session.exec(select(workbench_api_env.app_models.AttackStixTechnique)).all()
        mitigations = session.exec(select(workbench_api_env.app_models.AttackStixMitigation)).all()
        relationships = session.exec(
            select(workbench_api_env.app_models.AttackStixTechniqueMitigation)
        ).all()
