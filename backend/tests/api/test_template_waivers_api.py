...
def test_vpw064_template_waiver_lifecycle_and_report_visibility(
    ...
    asset_update = template_api_env.client.patch(
    ...
    accepted_detail = template_api_env.client.get(
    ...
    report = template_api_env.client.post(
        f"/api/v1/runs/{run_id}/reports",
        headers=headers,
        json={"format": "csv"},
    )
    assert report.status_code == 200, report.text
    csv_download = template_api_env.client.get(report.json()["download_url"], headers=headers)
    assert csv_download.status_code == 200
    assert "accepted" in csv_download.text
    assert "risk-owner" in csv_download.text
    assert "active" in csv_download.text

    updated = template_api_env.client.patch(
    ...
    expired = template_api_env.client.post(
    ...
    expired_detail = template_api_env.client.get(
        f"/api/v1/findings/{finding_id}",
        headers=headers,
    )
...
def test_vpw064_template_waiver_scopes_match_finding_cve_asset_and_service(
    template_api_env: TemplateApiEnv,
) -> None:
    headers = auth_headers(template_api_env.client)
    project = create_project_via_api(template_api_env.client, headers)
    ...
    listed = template_api_env.client.get(
        f"/api/v1/projects/{project['id']}/waivers/",
        headers=headers,
    )
...
def test_vpw064_template_waiver_validation_errors(template_api_env: TemplateApiEnv) -> None:
    ...
    missing_scope = template_api_env.client.post(
        f"/api/v1/projects/{project['id']}/waivers/",
        headers=headers,
        json={
            "owner": "risk",
    ...
    missing_expiry = template_api_env.client.post(
        f"/api/v1/projects/{project['id']}/waivers/",
        headers=headers,
        json={
            "cve_id": DEMO_CVE_LOG4SHELL,
    ...
    review_after_expiry = template_api_env.client.post(
        f"/api/v1/projects/{project['id']}/waivers/",
        headers=headers,
        json={
            "cve_id": DEMO_CVE_LOG4SHELL,
...
def _seed_report_run(
    ...
    *,
    ...
    with Session(template_api_env.engine) as session:
        run_repo = template_api_env.repositories.RunRepository(session)
        run = run_repo.create_analysis_run(
            project_id=project_id,
            input_type="generic-occurrence-csv",
            filename="waiver-report.csv",
            status=template_api_env.app_models.AnalysisRunStatus.COMPLETED,
